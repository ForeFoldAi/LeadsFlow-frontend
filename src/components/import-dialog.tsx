...
import type { CreateLeadDto, ImportLeadsResponse } from "@/lib/apis";
import { CustomerCategory, LeadSource, LeadStatus } from "@/lib/apis";
...
export default function ImportDialog({ onImportSuccess }: ImportDialogProps) {
  ...
  const processCSVFile = async (file: File) => {
    ...
    toast({
      ...
      description: `Reading and parsing ${isExcel ? 'Excel' : 'CSV'} file...`,
    });

    try {
      // Parse rows: Excel via SheetJS, CSV via PapaParse
      let rows: any[];
      if (isExcel) {
      ...
      const transformedLeads = rows
        ...
        .filter(lead => lead !== null) as CreateLeadDto[];
      ...
      try {
        const importResponse: ImportLeadsResponse = await leadsService.importLeads({
        ...
        const errorMessage = error?.response?.data?.message || error?.message || "Failed to import leads";
        toast({
          title: "Import Error",
      ...
      toast({
        title: "File Error",
        description: "Failed to read or parse the file. Please check the format and try again.",
        variant: "destructive",
      });
    } finally {
  ...
  const transformCSVRowToLead = (row: any, rowNumber: number): CreateLeadDto => {
    ...
    const validateEnum = (value: any, allowedValues: string[], defaultValue: string): string => {
    ...
    try {
      return {
        ...
        sector: cleanValue(row["Sector"] || row["sector"]) || undefined,
        customSector: cleanValue(row["Custom Sector (if custom)"] || row["Custom Sector"] || row["customSector"]) || undefined,
      };
    } catch (error) {
